...
const unavailable = new Set(["9:00 AM", "10:00 AM", "1:30 PM", "3:00 PM"]);
  ...
  if (submitted) {
    return (
      <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm text-center">
        ...
        <button
          onClick={() => { setSubmitted(false); setSelectedTime(""); setForm({ name: "", email: "", phone: "", notes: "" }); }}
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      ...
      {selectedTime && (
        <form onSubmit={handleSubmit} className="space-y-3 border-t border-slate-100 pt-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">Brief case description (optional)</label>
            <textarea
              rows={2}
