...
export const defaultCaseTypes: Record<string, CaseType[]> = {
...
export interface SOLCalculatorProps {
  caseTypes?: Record<string, CaseType[]>;
}

export default function SOLCalculator({ caseTypes = defaultCaseTypes }: SOLCalculatorProps) {
  ...
  const calculate = () => {
    ...
    if (selectedCase.years === 0) {
      setResult(null);
      alert("This offense may be imprescriptible or follow special rules. Please consult Atty. Baligod for specifics.");
      return;
    }
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="space-y-4">
        {/* Category */}
        <div>
          <label className="block text-xs font-semibold text-slate-600 mb-1.5">Case Category</label>
          <select
            value={category}
        ...
        {caseType && (
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Date of Incident</label>
            <input
              type="date"
              value={incidentDate}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">
                  Filing Deadline:{" "}
                  <span className={result.daysLeft <= 0 ? "text-red-600" : "text-green-700"}>
                ...
                <p className="text-sm mt-1">
                  {result.daysLeft <= 0 ? (
                    ...
                    <span className="text-slate-600">
                      You have{" "}
                      <strong className={result.daysLeft <= 90 ? "text-amber-600" : "text-green-700"}>
