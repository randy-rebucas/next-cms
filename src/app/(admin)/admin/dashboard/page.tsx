            ...
            {recent.map((p) => (
              <li key={p._id} className="flex items-center justify-between px-5 py-3 hover:bg-slate-800/50 group">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500">{p.author_name || "—"} · {new Date(p.createdAt).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0 ml-4">
                  ...
                  <Link href={`/admin/posts/${p._id}/edit`} className="text-xs text-amber-500 hover:text-amber-400">Edit</Link>
                </div>
              </li>
            ))}
          </ul>
