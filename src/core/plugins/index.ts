...
/**
 * Runtime plugin manifest — every plugin package must export one of these
 * as its default export so bootstrap.ts can register it.
 *
 *   export const myPlugin: PluginManifest = {
 *     id: "my-plugin",
 *     register() { registerHook("beforeSavePost", ...); },
 *   };
 */
export interface PluginManifest {
  id: string;
  /** Called once by bootstrap when the plugin is enabled. */
  register: () => void;
}
